...
class Shader {
    ...
    set(location: number, type: string, value: any) {
        switch(type) {
            case "float":
    ...
    use() { this.#gl.useProgram(<WebGLProgram>this.program); }
    ...
    #initProgram  (vSource: string, fSource: string) {
        const vertexShader = <WebGLShader>this.#loadAndCompileShader(this.#gl.VERTEX_SHADER, vSource);
        const fragmentShader = <WebGLShader>this.#loadAndCompileShader(this.#gl.FRAGMENT_SHADER, fSource);
        ...
        for(let line of source) {
            if(line !== "") {
                if(line.startsWith("attribute")) {
                    ...
                    attributes[name] = {
                        location: this.#gl.getAttribLocation(<WebGLProgram>this.program, name),
                        type,
                    };
                }
                    ...
                    const [, type, name] = line.split(" ");
                    uniforms[name] = {
                        location: this.#gl.getUniformLocation(<WebGLProgram>this.program, name),
                        type
                    };
                }
            }   // if line ends
