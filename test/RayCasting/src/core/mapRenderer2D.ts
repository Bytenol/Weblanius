...
class Vertex {
    ...
    color: vec4;
    texCoord: vec2 = [0, 0];
    constructor(pos: vec2 = [0, 0], col: vec4 = [0, 0, 0, 1], tex: vec2 = [0, 0] ) {
        this.position = pos;
        this.color = col;
        this.texCoord = tex;
    }
...
const createQuad = (x: number, y: number, c: vec4) => {
    let size = 1;
    return [
...
class MapRenderer2D {
    ...
    get domElement(){ return <HTMLCanvasElement>this.#canvas; }
    ...
    init(): boolean {
        ...
        this.#gl = <WebGLRenderingContext>this.domElement.getContext("webgl");
