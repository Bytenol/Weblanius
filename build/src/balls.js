...
class Ball {
    mass = Math.random() * 8 | 0x0;
    radius = 20; //(Math.random() * 4 + 10) | 0;
    pos = new Vec2(0, 0);
    vel = new Vec2(0, 0);
    color = (Math.random() * 40234 + 10543) | 0;
    isMovable = true;
    id = "xxx";
    constructor() {
        // Ball.all.push(this);
...
class Wall {
    ...
    render(ctx) {
        ...
        drawLine(ctx, this.start.addScaled(this.normal, -this.radius * 2), this.end.addScaled(this.normal, -this.radius * 2), "#000");
        ctx.closePath();
        this.b1.render(ctx);
        this.b2.render(ctx);
    }
...
const findResultantForce = (forces) => {
...
const update = (dt, ctx) => {
    for (let i = 0; i < Ball.all.length; i++) {
        const b1 = Ball.all[i];
        if (b1.isMovable)
            b1.pos = b1.pos.addScaled(b1.vel, dt);
        // clamp position
