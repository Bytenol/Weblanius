...
const initSceneAnchor = () => {
    ...
    [...anchors].forEach((anchor) => {
        anchor.addEventListener("click", (e) => {
            e.preventDefault();
            const name = anchor.children[1].textContent.toLocaleLowerCase();
            toggleScene(name + "Scene");
        });
    });
};
const setAge = () => {
    const spanEl = document.getElementById("myAge");
...
const animateBirthday = (ctx) => {
    ...
    class Particle {
        pos = new Vec2(ctx.canvas.width / 2, ctx.canvas.height / 2);
        posZ = 50.0;
        color = `rgb(${randRange(0, 255)}, ${randRange(0, 255)}, ${randRange(0, 255)})`;
        vel = new Vec2(randRange(-100, 100), randRange(-100, 100));
        constructor() { }
        get size() {
