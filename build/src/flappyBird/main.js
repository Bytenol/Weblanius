...
const draw = () => {
    ...
    if (game.state == game.stateMode.IDLE) {
        ...
        game.drawAt(program, 290, 58, 100, 26, gl.canvas.width * 0.7, gl.canvas.width * 0.5 - gl.canvas.width * 0.35, gl.canvas.height * 0.3);
        game.drawAt(program, 300, 90, 40, 20, gl.canvas.width * 0.4, gl.canvas.width * 0.5 - gl.canvas.width * 0.2, gl.canvas.height * 0.5);
        game.drawAt(program, 290, 120, 60, 38, gl.canvas.width * 0.8, gl.canvas.width * 0.5 - gl.canvas.width * 0.4, gl.canvas.height * 0.7);
    }
    else if (game.state == game.stateMode.OVER) {
        // over
        game.drawAt(program, 390, 58, 105, 28, gl.canvas.width * 0.7, gl.canvas.width * 0.5 - gl.canvas.width * 0.35, gl.canvas.height * 0.2);
        game.drawAt(program, 350, 116, 60, 32, gl.canvas.width * 0.4, gl.canvas.width * 0.5 - gl.canvas.width * 0.2, gl.canvas.height * 0.35);
        bird.draw(program);
    }
    else {
        // active
...
const main = () => {
    ...
    sprite.addEventListener("error", (e) => {
        console.log(e);
    });
};
